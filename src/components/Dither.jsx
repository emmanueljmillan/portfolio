...
float pattern(vec2 p) {
  vec2 movement = vec2(time * waveSpeed, time * waveSpeed * 0.35);
  float n1 = fbm(p + movement);
  float n2 = fbm(p * 1.6 - movement);
  ...
  useFrame((state, delta) => {
    ...
    if (!disableAnimation) {
      ...
      if (materialRef.current) {
        materialRef.current.uniforms.time.value = u.time.value
      }
    }
